...
- **`app.js`** — El "shell": barra de navegación inferior, `draw(route)` que conmuta entre vistas, botón flotante (+) en Movimientos, manejo de sesión (`startSession`/`stopSession`/`liveRefresh`), recordatorio de respaldo y **recordatorios de vehículo** (`computeReminders`: obligaciones por vencer + mantenimientos que tocan por km/fecha → badge numérico en "Más" y notificación diaria; el badge se recalcula al salir de Vehículos).
