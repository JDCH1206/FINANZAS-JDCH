...
export async function loadData(uid) {
  ...
  if (!raw) return { profile: { name: "", income: 6000000 }, cats: structuredClone(DEFAULT_CATS), budgets: {}, accounts: [], payMethods: [], vehiclesEnabled: false, vehicles: [], goals: [], recurrentes: [], txs: [], incomes: [], isNew: true };
...
export async function addTx(uid, tx) {
  if (FIREBASE_READY) {
    ...
    await fsMod.setDoc(ref, { date: tx.date, desc: tx.desc, amount: tx.amount, cat: tx.cat, sub: tx.sub, pay: tx.pay || "", acct: tx.acct || "", vehicleId: tx.vehicleId || "", fuelId: tx.fuelId || "", maintId: tx.maintId || "", obligId: tx.obligId || "", tags: tx.tags || [], splitId: tx.splitId || "" });
...
export async function bulkUpdateTx(uid, txs) {
  ...
  for (const tx of txs) {
    batch.set(fsMod.doc(db, "users", uid, "transactions", tx.id), { date: tx.date, desc: tx.desc, amount: tx.amount, cat: tx.cat, sub: tx.sub, pay: tx.pay || "", acct: tx.acct || "", vehicleId: tx.vehicleId || "", fuelId: tx.fuelId || "", maintId: tx.maintId || "", obligId: tx.obligId || "", tags: tx.tags || [], splitId: tx.splitId || "" });
